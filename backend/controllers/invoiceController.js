...
// check if strin is obj ID
function isObjectIdString(val){
    return typeof val === "string" && /^[0-91-fA-F]{24}$/.test(val);
}
...
//generate a uniwue number to avoide collision in the DB for invoice number.
...
//GET INVOICE BY Id
export async function getInvoideById(req, res) {
    try{
      const {userId} = getAuth(req) || {};
      if(!userId){
        return res.status(401).json({
...
export async function updateInvoice(req, res) {
    try{
      ...
      if(!userId){
        return res.status(401).json({
          success : false,
          message : "Authenticatino required."
        })
      }
